...
**TL;DR:** In this article, we'll implement an Audio player using **Angular** and **Ionic**. We will handle Audio Operations using **RxJS** _Observables_ and we'll also explore how we can manage application's state with **NgRx** inside an Angular application. We will also use **Auth0** for Authentication. You can check the final code at [Github Repository](https://github.com/imsingh/audio-player/).
...
* Instal the Native sdks for building application.

  * For iOS applications, you need to install XCode. Check the complete requirements [here](https://cordova.apache.org/docs/en/latest/guide/platforms/ios/#installing-the-requirements). It's important to keep in mind that, you need macOS to install xCode.
...
In order to make our app communicate with Auth0, we need to configure Auth0 App and we need certain Information.
...
Let's create a file _store.ts_ in _/src/providers/store/_ folder and do the following:
...
Let's create a file _auth.config.ts_ in _/src/providers/auth0/_ folder and save the configuration as follows:
...
Let's create _auth.service.ts_ in the same _auth0_ folder and add the following content.
...
Let's first understand the properties of this Auth Service:
...
Finally we also allow user to logout via logout method.
