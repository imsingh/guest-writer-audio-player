...
```bash
cd audio-player
npm install --save @angular/animations @ngrx/store moment auth0-js @types/auth0-js rxjs@6.2.1 rxjs-compat@6.2.1
```

> **Note**: As of writing this article, when you create a new Ionic application. It is shipped with Angular 5.x and RxJS 5.x. But the latest `@ngrx/store` version(6.x)  requires RxJS 6.x. That's why you need to install version 6.x of RxJS. Also, Angular 5.x uses RxJS 5.x APIs. So to have backward compatibility with it, you also need to install `rxjs-compat`.
...
This will generate a service in file called `audio.ts` under `./src/providers/audio/` and this service will be added to `NgModule` in `app.module.ts`. Replace the contents of this file with:
...
Now that you have created Audio Service for playback features. You will need to create a service to get list of audio files.

You can create a cloud service using Ionic:

```bash
ionic generate provider cloud
```
It will generate a service in file called `cloud.ts` under `./src/providers/cloud`. Replace the content of this file with:
