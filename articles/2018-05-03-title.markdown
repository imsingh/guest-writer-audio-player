...
**TL;DR:** In this article, you will learn how to implement a mobile audio player app using [**Angular**](https://angular.io) and [**Ionic**](https://ionic.io). You will handle audio operations using [_RxJS_](https://github.com/ReactiveX/rxjs) and _Observables_ and you will also explore how you can manage application's state with [**NgRx**](https://github.com/ngrx/platform). To secure your application, you will use [**Auth0**](https://auth0.com). If needed, [you can find the final code at this GitHub repository](https://github.com/imsingh/audio-player/).
...
```bash
cd audio-player
npm install --save @angular/animations @ngrx/store moment auth0-js @types/auth0-js @auth0/cordova @auth0/angular-jwt
```
...
Now that you understand why do you need an observer, you can start by creating a service in your Ionic app:

```bash
ionic generate provider audio
```

This will generate a file called `audio.ts` under `./src/providers/audio/`. Replace the contents of this file with:
...
@imsingh, add instructions here on how to set up your mock `CloudProvider`. Please, use the second-person singular narrative.
...
Continues with the definition of a factory function to configure the `angular-jwt` library:
...
@imsingh, is the `whitelistedDomains` needed now that we won't use Webtask in this article?
...
> Take a closer look at how the `JwtModule` is configured.
...
@imsingh, please, write what have been accomplished in this article